...
In the revised manuscript, we clarify that we chose the \emph{oldest} laboratory value within the two-year window to approximate a screening-like deployment scenario, reduce peri-diagnostic bias from acute work-up, and maintain a simple, comparable tabular representation across cancer cohorts (Methods, pp.~3--4, lines~132--141).
We also now explicitly acknowledge that collapsing longitudinal trajectories into a single snapshot may attenuate discriminative performance and preferentially include patients with more complete laboratory histories, and we discuss these limitations in the Discussion (p.~15, lines~565--578).
...
First, in the Statistical Analysis subsection of the Materials and Methods, we now describe how we computed feature attributions using DeepLIFT-based SHAP values for the neural network model (p.7, lines~284--290).
Second, we added a new Results subsection entitled ``Feature importance patterns (SHAP)'' (p.~12, lines~417--426) and a corresponding figure (Figure~\ref{fig:shap-global}).
Third, we expanded the Discussion to interpret these patterns (p.~14, lines~499--513).
...
We thank the reviewer for highlighting this important point. In the revised manuscript, we have expanded the Discussion to provide a more explicit analysis of the selection bias induced by the lab-complete inclusion criterion (pp.~15--16, lines~579--595).
...
We appreciate this insightful suggestion. In the revised manuscript, we have added a dedicated paragraph in the \textit{Introduction} (p.~2, paragraph~5, lines~73--81) to situate our work within the landscape of recent self-supervised and foundation-model research for EHRs.
%\begin{quote}
...
Finally, I encourage the authors to provide explicit information on code availability or reproducibility plans. Since the dataset (MIMIC-IV) is publicly accessible, releasing model code or training scripts would enable full replication of results, which is increasingly expected in computational medicine research. This would also enhance the manuscript’s transparency and impact.

\subsection*{Response 8}
We thank the reviewer for highlighting the importance of reproducibility and code availability. In the revised manuscript, we now explicitly describe our plans to release the implementation used in this study (p.~7, lines~300--303).
