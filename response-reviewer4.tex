...
We fully agree with the reviewer that the lack of external validation is a major limitation and that it constrains the generalizability of our findings beyond the MIMIC-IV cohort.
%At present, we do not have access to an independent multi-center or outpatient EHR dataset under appropriate data use agreements, and designing a meaningful domain-shift emulation (e.g., cross-year or stratified splits that reflect real differences in practice patterns) would require substantial work beyond the scope of this revision.
At present, we do not have access to an independent multi-center or outpatient EHR dataset under appropriate data use agreements, and within the constraints of this revision we were unfortunately not able to carefully design and evaluate a domain-shift emulation (e.g., cross-year or stratified splits that reflect real differences in practice patterns).
We agree that such analyses would substantially strengthen the work and regard them as a priority for follow-up studies.
In light of this, we have substantially expanded our discussion of this limitation and its implications for deployment.

Specifically, in the Discussion we now (i) explicitly describe MIMIC-IV as a single-center, predominantly inpatient database and discuss how differences in patient mix, coding practices, and endoscopy referral thresholds may limit generalizability to outpatient and multi-institutional settings, and (ii) emphasize that the performance estimates reported here should not be assumed to transfer to other institutions without rigorous external validation and recalibration (p.~X, paragraph~Y, lines~A--B).
...
Finally, we have toned down language elsewhere in the manuscript that could be interpreted as claiming generalizability beyond MIMIC-IV.
